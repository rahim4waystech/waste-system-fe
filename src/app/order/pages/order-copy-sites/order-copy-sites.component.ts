...
@Component({
  selector: 'app-order-copy-sites',
  templateUrl: './order-copy-sites.component.html',
  styleUrls: ['./order-copy-sites.component.scss']
})
export class OrderCopySitesComponent implements OnInit {
  accountId: number = -1;
...
addSite() {
  ...
  select(button, record): void {
    if(!record.accountRef || record.accountRef.trim() === '') {
      ...
      this.router.navigateByUrl('/orders/copy/' + this.order.id + '/' + this.accountId + '/' + record.id);
    }
  }
