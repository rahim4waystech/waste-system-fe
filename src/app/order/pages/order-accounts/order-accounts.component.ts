...
@Component({
  selector: 'app-order-accounts',
  templateUrl: './order-accounts.component.html',
  styleUrls: ['./order-accounts.component.scss']
})
export class OrderAccountsComponent implements OnInit {
  ...
  select(button, record): void {
    if(!record.accountRef || record.accountRef.trim() === '') {
      ...
      this.router.navigateByUrl('/orders/new/accounts/' + record.id + '/sites');
    }
  }
