...
@Component({
  selector: 'app-order-copy-accounts',
  templateUrl: './order-copy-accounts.component.html',
  styleUrls: ['./order-copy-accounts.component.scss']
})
export class OrderCopyAccountsComponent implements OnInit {
  ...
  select(button, record): void {
    if(!record.accountRef || record.accountRef.trim() === '') {
      ...
      this.router.navigateByUrl('/orders/copy/change/' + this.orderId + '/accounts/' + record.id + '/sites');
    }
  }
