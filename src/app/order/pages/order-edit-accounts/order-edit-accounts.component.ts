...
@Component({
  selector: 'app-order-edit-accounts',
  templateUrl: './order-edit-accounts.component.html',
  styleUrls: ['./order-edit-accounts.component.scss']
})
export class OrderEditAccountsComponent implements OnInit {
  ...
  select(button, record): void {
    if(!record.accountRef || record.accountRef.trim() === '') {
      ...
      this.router.navigateByUrl('/orders/change/' + this.orderId + '/accounts/' + record.id + '/sites');
    }
  }
