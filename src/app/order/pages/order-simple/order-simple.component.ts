...
@Component({
  selector: 'app-order-simple',
  templateUrl: './order-simple.component.html',
  styleUrls: ['./order-simple.component.scss']
})
export class OrderSimpleComponent implements OnInit {
  ...
  warningForCreditLimit: boolean = false;

  isSimpleOrder: boolean = true;
  ...
  onSubmit() {
    ...
    if(this.order.orderTypeId === 8 && this.order.shredderOrderTypeId === 3){
      ...
      let deliveryLines = JSON.parse(JSON.stringify(this.orderLines));
      let returnLines = JSON.parse(JSON.stringify(this.orderLines));
