...
@Component({
  selector: 'app-job-signoff-shredder-transport',
  templateUrl: './job-signoff-shredder-transport.component.html',
  styleUrls: ['./job-signoff-shredder-transport.component.scss']
})
export class JobSignoffShredderTransportComponent implements OnInit {
  ...
  buttons: GridButton[] = [
    {label: 'Sign off', link: '/job-signoff/shredder/view/:id', type: 'btn-primary', trigger: this.isSighOffButtonVisible} as GridButton,
    // {label: 'Get destruction certificate', action: (button, record) => {this.onPDFDownload(record);}, type: 'btn-danger'} as GridButton,
    {label: 'Get destruction certificate', trigger: (record) => record.order.shredderOrderTypeId !== 3 || (record.order.shredderOrderTypeId === 3 && record.order.isPrepaid === true), action: (button, record) => {this.onPDFDownload(record);}, type: 'btn-danger'} as GridButton,
