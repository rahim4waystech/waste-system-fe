...
@Component({
  selector: 'app-artic-run-diary-set-allocation-modal',
  templateUrl: './artic-run-diary-set-allocation-modal.component.html',
  styleUrls: ['./artic-run-diary-set-allocation-modal.component.scss']
})
export class ArticRunDiarySetAllocationModalComponent implements OnInit {
  ...
  save() {
    ...
    let dateTest = moment(this.job.date, "L").format('YYYY-MM-DD');

    this.jobAssignmentService.checkIfJobAssignmentExistsForDate(dateTest, this.job.jobAssignment.driverId, this.job.jobAssignment.vehicleId)
    .pipe(take(1))
    .pipe(catchError((e) => {
  ...
  getNextSlotId() {
    let dateTest = moment(this.job.date, "L").format('YYYY-MM-DD');
    this.jobAssignmentService.getNextSlotId(dateTest, this.orderTypeId)
    .pipe(take(1))
    .pipe(catchError((e) => {
  ...
  saveJob() {
    ...
    .subscribe((data) => {
      ...
      this.modalService.close('articSetAllocationModal');
