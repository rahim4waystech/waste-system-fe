...
@Component({
  selector: 'app-weighbridge-yard-trade-edit',
  templateUrl: './weighbridge-yard-trade-edit.component.html',
  styleUrls: ['./weighbridge-yard-trade-edit.component.scss']
})
export class WeighbridgeYardTradeEditComponent implements OnInit {

  customer: Account = new Account();
  depots: Account[] = [];
  yardTrade: any = new YardTrade();
  ...
  loadYardTard(yardTradeId: number) {
    this.yardTradeService.getYardTradeByIdExpanded(yardTradeId)
    .pipe(take(1))
    .pipe(catchError((e) => {
      if(e.status === 403 || e.status === 401) {
        return e;
      }
      alert('Could not load yard trade');
      return e;
    }))
    .subscribe((yardTrade: YardTrade) => {
      this.yardTrade = <any> yardTrade;
      this.customer = this.yardTrade.customer;
      this.yardTradePricing = <any>(this.yardTrade.yardTradePricing);
      this.loadSites();
    })
  }
  ...
  save() {
    ...
    this.yardTradeService.updateYardTrade(this.yardTrade)
    .pipe(take(1))
    .pipe(catchError((e) => {
    ...
    .subscribe((data: YardTrade) => {
      ...
      this.yardTradePricing.forEach((price: YardTradePricing) => {
        price.id = -1;
        price.yardTrade = {id: data.id} as any;
        price.yardTradeId = data.id;
      }) 
      this.bulkDeletePricing();
    })
  ...
  bulkDeletePricing() {
    if(!this.yardTradePricing) {
      this.isServerError = true;
    }

    this.yardTradeService.deleteAllPricingForYardTradeId(this.yardTrade.id)
    .pipe(take(1))
    .pipe(catchError((e) => {
      this.isServerError = true;
      return e;
