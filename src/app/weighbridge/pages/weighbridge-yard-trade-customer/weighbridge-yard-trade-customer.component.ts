...
@Component({
  selector: 'app-weighbridge-yard-trade-customer',
  templateUrl: './weighbridge-yard-trade-customer.component.html',
  styleUrls: ['./weighbridge-yard-trade-customer.component.scss']
})
export class WeighbridgeYardTradeCustomerComponent implements OnInit {
  ...
  select(button, record): void {
    if(!record.accountRef || record.accountRef.trim() === '') {
      ...
      this.router.navigateByUrl('/weighbridge/yard-trade/' + record.id);
    }
  }
