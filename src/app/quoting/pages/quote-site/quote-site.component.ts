...
@Component({
  selector: 'app-quote-site',
  templateUrl: './quote-site.component.html',
  styleUrls: ['./quote-site.component.scss']
})
export class QuoteSiteComponent implements OnInit {
  ...
  select(button, record): void {
    if(!record.accountRef || record.accountRef.trim() === '') {
      ...
      this.router.navigateByUrl('/quoting/new/accounts/' + record.parentId + '/sites/' + record.id);
    }
  }
