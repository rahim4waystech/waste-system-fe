...
@Component({
  selector: 'app-quote-customer',
  templateUrl: './quote-customer.component.html',
  styleUrls: ['./quote-customer.component.scss']
})
export class QuoteCustomerComponent implements OnInit {
  ...
  select(button, record): void {
    if(!record.accountRef || record.accountRef.trim() === '') {
      ...
      this.router.navigateByUrl('/quoting/new/accounts/' + record.id + '/sites');
    }
  }
