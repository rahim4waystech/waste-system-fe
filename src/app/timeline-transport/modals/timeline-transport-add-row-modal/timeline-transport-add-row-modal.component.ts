...
@Component({
  selector: 'app-timeline-transport-add-row-modal',
  templateUrl: './timeline-transport-add-row-modal.component.html',
  styleUrls: ['./timeline-transport-add-row-modal.component.scss']
})
export class TimelineTransportAddRowModalComponent implements OnInit {
  ...
  updateNameFromVehicle() {
    const vehicle = this.vehicles.filter(v => v.id === this.jobAssignment.vehicleId)[0];
  ...
  cancel() {
    this.reset();
    this.modalService.close('transportRowCreationModal');
  }
  ...
  createJobAssignment(jobAssignment: JobAssignment) {
    ...
    this.jobAssignmentService.bulkCreateJobAssignment(objects)
    .pipe(take(1))
    .pipe(catchError((e) => {
