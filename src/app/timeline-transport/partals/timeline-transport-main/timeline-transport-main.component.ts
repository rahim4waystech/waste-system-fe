...
import { TransportTimelineSortOrderService } from '../../services/transport-timeline-sort-order.service';
import { TimelineHistoryEvent, TimelineHistoryEventType, TimelineTransportHistoryService } from '../../services/timeline-transport-history.service';
import { TransportTimelineHistoryActionService } from '../../services/transport-timeline-history-action.service';
import { Driver } from 'src/app/driver/models/driver.model';

declare var $: any;

@Component({
  selector: 'app-timeline-transport-main',
  templateUrl: './timeline-transport-main.component.html',
  styleUrls: ['./timeline-transport-main.component.scss']
})
export class TimelineTransportMainComponent implements OnInit {
  ...
  openOrderModal(job: Job) {
    ...
    this.openModal("jobTransportInformationModal");
  }
  loadJobAssignment() {
...
// Disable to rewrite as it's another db call in a lo0op
  ...
  openEditModal(assignment: JobAssignment) {
    ...
    this.modalService.open('transportRowEditModal');
  }
...
debugger;
  ...
  assignment.jobs = assignment.jobs.filter(j => j.id !== job.id);
debugger;
  this.modalService.open('transportJobCreationModal');
  }
...
debugger
  ...
  onAddAssignmentNoteClicked() {
    ...
    this.modalService.open('transportNoteAssignmentCreationModal');
  }
  ...
  copyJobs(jobAssignment: JobAssignment) {
    ...
    this.modalService.open('transportCopyJobsModal');
  }
