...
@Component({
  selector: 'app-timeline-transport-toolbar',
  templateUrl: './timeline-transport-toolbar.component.html',
  styleUrls: ['./timeline-transport-toolbar.component.scss']
})
export class TimelineTransportToolbarComponent implements OnInit {
  ...
  addRow() {
    this.modalService.open('transportRowCreationModal');
  }

  showVorAndAbsence() {
    this.modalService.open('vorModal');
  }
  ...
  onAddNoteClicked() {
    ...
    this.modalService.open('transportNoteCreationModal');
  }
  onApproveJobClicked() {
    if(this.selectedJobs.length === 0) {
  ...
  onEditJobClicked() {
    ...
    this.modalService.open('transportJobEditModal');
  }
