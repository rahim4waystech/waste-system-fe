...
@Component({
  selector: 'app-contact-account-modal',
  templateUrl: './contact-account-modal.component.html',
  styleUrls: ['./contact-account-modal.component.scss']
})
export class ContactAccountModalComponent implements OnInit {
  ...
  cancel() {
    this.modalService.close('contactAccountModal');
    this.accounts = [];
  }

  onCheckedSelected($event) {
    if($event.checked) {
      this.accounts.push($event.record);
    } else {
      this.accounts = this.accounts.filter(j => j.id !== $event.record.id);
    }
  }

  save() {

    this.selectedAccounts.emit(JSON.parse(JSON.stringify(this.accounts)));
    this.cancel();
  }
