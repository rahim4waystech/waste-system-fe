...
export class ActiveAccountsComponent implements OnInit {
  ...
  buttons: GridButton[] = [
   {label: 'Add callback', action: (button, record) => { this.currentAccount = record; this.modalService.open('addCallbackModal'); }, link: '', type: 'btn-info',} as GridButton,
  // { 
  //   label: 'Add callback', 
  ...
  {label: 'Add Opportunity', trigger: (record, button) => ((record.type_id === 3 || record.type_id === 16 || record.type_id === 8) && environment.crmOptions.showOpps), link: '/opportunities/account/:id', type: 'btn-info',} as GridButton,
