...
@Component({
  selector: 'app-artic-timeline-transport',
  templateUrl: './artic-timeline-transport.component.html',
  styleUrls: ['./artic-timeline-transport.component.scss']
})
export class ArticTimelineTransportComponent implements OnInit {

  absenceAndVors: any = {vor: [], absence: []};
  date: string = moment().format('YYYY-MM-DD');
  orderTypeId: number = 4; //artic maps to trailer
  ...
  loadVehicleVOR() {
    ...
    this.vehicleVORService.getVehicleVORsByDateAndVehicleTypeId(moment(this.date).format('YYYY-MM-DD'),6)
    .pipe(take(1))
    .pipe(catchError((e) => {
      if(e.status === 403 || e.status === 401) {
        return e;
