...
@Component({
  selector: 'app-job-signoff-artic-view',
  templateUrl: './job-signoff-artic-view.component.html',
  styleUrls: ['./job-signoff-artic-view.component.scss']
})
export class JobSignoffArticViewComponent implements OnInit {
  ...
  saveJob() {
    ...
    .subscribe(() => {
      ...
      const signoffList = localStorage.getItem('WS_MJL_ARTIC_SIGNOFF_LIST');
      if(!signoffList || signoffList === "") {
        ...
        if(signOffListObj.length !== 0) {
          const url = "/job-signoff/artic/view/" + signOffListObj[0].id;
          localStorage.setItem('WS_MJL_ARTIC_SIGNOFF_LIST', JSON.stringify(signOffListObj));
  ...
  cancel() {
    localStorage.setItem('WS_MJL_ARTIC_SIGNOFF_LIST', "");
    window.location.href = '/job-signoff/artic';
